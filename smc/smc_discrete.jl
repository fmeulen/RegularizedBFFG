...
function Pg_params(x, vt, p)
    @unpack ω, ψ, η, μ, σ² = p
    σ²_g = 1.0 / (1.0/η^2 + 1.0/σ²)
    μ_g  = σ²_g * ((ω + ψ*x)/η^2 + (vt - μ)/σ²)
    return μ_g, sqrt(σ²_g)
end
...
function sample_twisted(x, vt, ε, p, rng)
    ...
    α  = ct / (ct + ε)
    if rand(rng) < α
        μ_g, σ_g = Pg_params(x, vt, p)
        return randn(rng) * σ_g + μ_g
    else
...
function compare_adaptive(;
    ...
    for (label, η_val, slug) in [("paste(eta == $(η_good), ' (< ', sigma, ')')", η_good, "good"),
                                  ("paste(eta == $(η_bad), ' (> ', sigma, ')')",  η_bad,  "bad")]
    ...
    p_ess <- ggplot(df_ess, aes(x = value, y = cdf, colour = method, linetype = method)) +
      ...
      facet_wrap(~ eta, scales = "free_x", labeller = label_parsed) +
...
function run_smc_result(p; T=50, N=500, ε=0.01, seed=42, adaptive=false)
    ...
    if adaptive
        log_liks, ess_vec, _, _, _, ε_vec, _ = run_smc_adaptive_optim(vs, N, p; rng=rng)
    else
...
"""
function plot_smc_runs_faceted(runs, seed)
    ...
    p_ess <- ggplot(df, aes(x = t, y = ess, colour = method)) +
      geom_line(linewidth = 0.8) +
      facet_wrap(~ eta, labeller = label_parsed) +
      labs(x = "t", y = "ESS", colour = "Method", title = "") +
      theme_bw(base_size = 12) +
      theme(strip.background = element_blank()) +
      theme(legend.position="bottom")
    ggsave(paste0("figs/smc_run_ess",seed,".pdf"), p_ess, width = 9, height = 4)
    ...
    p_cum <- ggplot(df, aes(x = t, y = cum_ll, colour = method)) +
      ...
      theme_bw(base_size = 12) +
      theme(strip.background = element_blank())
    ggsave(paste0("figs/smc_run_cumloglik",seed,".pdf"), p_cum, width = 9, height = 4)
    ...
    p_inc <- ggplot(df, aes(x = t, y = inc_ll, colour = method)) +
      ...
      theme_bw(base_size = 12) +
      theme(strip.background = element_blank())
    ggsave(paste0("figs/smc_run_incloglik",seed,".pdf"), p_inc, width = 9, height = 4)
    ...
    if (length(eps_v) > 0) {
      ...
      p_eps <- ggplot(df_eps, aes(x = t, y = eps_star, colour = method)) +
        ...
        theme_bw(base_size = 12) +
        theme(strip.background = element_blank())
      ggsave(paste0("figs/smc_run_epsstar",seed,".pdf"), p_eps, width = 9, height = 4)
    }
    """
...
p = Para()
main_smc(p)
compare_adaptive(;T         = 50,
        N        = 500,
        R        = 1000,
        η_good   = 1.0,
        η_bad    = 4.0,
        ε_range  = [0.0, 0.0001, 0.01, 0.1],
        seed     = 42)
...
myseed = 55 # this one is nice!
runs = [
    (label="eta == 1.0", method="fixed (eps=0)", result=run_smc_result(Para(η=1.0); adaptive=false, ε=0.0, seed=myseed)),
    (label="eta == 1.0", method="adaptive",      result=run_smc_result(Para(η=1.0); adaptive=true, seed=myseed)),
    (label="eta == 4.0", method="fixed (eps=0)", result=run_smc_result(Para(η=4.0); adaptive=false, ε=0.0, seed=myseed)),
    (label="eta == 4.0", method="adaptive",      result=run_smc_result(Para(η=4.0); adaptive=true, seed=myseed)),
]
plot_smc_runs_faceted(runs, myseed)

myseed = 222 # this one is nice!
runs = [
    (label="eta == 1.0", method="fixed (eps=0)", result=run_smc_result(Para(η=1.0); adaptive=false, ε=0.0, seed=myseed)),
    (label="eta == 1.0", method="adaptive",      result=run_smc_result(Para(η=1.0); adaptive=true, seed=myseed)),
    (label="eta == 4.0", method="fixed (eps=0)", result=run_smc_result(Para(η=4.0); adaptive=false, ε=0.0, seed=myseed)),
