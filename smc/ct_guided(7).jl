...
@with_kw struct ParaCT{R}
    ...
    α::R     = 0.5       # nonlinear oscillation amplitude
    q::R     = 1.0       # diffusion coefficient
    # Observation
    v::R     = 1.0       # observed value
    σ_obs::R = 1.0       # observation noise std
    # Time grid
    T::R     = 1.0       # terminal time
    n_steps::Int = 1000  # number of Euler-Maruyama steps
end
...
Returns: (xT, log_psi, xs)
"""
"""
Simulate guided path with either a pre-drawn noise vector or an rng.
If dWs is provided, uses those increments (allows reuse across ε values).
"""
...
function estimate_m(ε, N, p, rng)
    ...
    return mean(weights .^ 2) * (g_ct(0.0, p.x0, p) + ε)^2 / (g_ct(0.0, p.x0, p) + ε)^2
end
...
function main(; N=500, R=100, seed=42)
    ...
    εs = exp.(range(log(1e-5), log(2.0), length=60))
