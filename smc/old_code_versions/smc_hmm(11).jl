...
function run_smc(vs, N, ε, p; rng=Random.default_rng(), ess_min=N/2)
    ...
    return log_liks, ess_vec, minimum(ess_vec), X
end
...
function main_smc(p; T=50, N=500, ε=0.01, seed=42)
    ...
    log_liks, ess_vec, _, particles = run_smc(vs, N, ε, p; rng=rng)
...
function _run_smc_adaptive(vs, N, p, find_eps_fn; rng=Random.default_rng(), ess_min=N/2)
    ...
    return log_liks, ess_vec, minimum(ess_vec), ε_vec, X
end
    ...
    for (label, η_val, slug) in [("eta=$(η_good) < sigma", η_good, "good"),
        ...
        for r in 1:R
            for ε in ε_range
                ...
                _, _, min_ess_f, _ = run_smc(vs, N, ε, p; rng=rng_r)
                push!(miness_fixed[ε], min_ess_f)
            end

            rng_r2 = MersenneTwister(seed + r)
            _, _, min_ess_g, ε_vec_g, _ = run_smc_adaptive(vs, N, p; rng=rng_r2)
            push!(miness_grid, min_ess_g)
            push!(eps_grid, ε_vec_g)

            rng_r3 = MersenneTwister(seed + r)
            _, _, min_ess_o, ε_vec_o, _ = run_smc_adaptive_optim(vs, N, p; rng=rng_r3)
...
function run_and_plot(p; T=50, N=500, ε=0.01, seed=42, adaptive=false)
    ...
    if adaptive
        log_liks, ess_vec, _, ε_vec, _ = run_smc_adaptive(vs, N, p; rng=rng)
        plot_smc_run(vs, xs_true, log_liks, ess_vec, slug; ε_vec=ε_vec)
    else
        log_liks, ess_vec, _, _ = run_smc(vs, N, ε, p; rng=rng)
