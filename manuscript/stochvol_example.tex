...
It follows that for $\eta\ge \pi/\sqrt{2}\approx 2.221$ the estimator $\hat L$ has infinite variance. One possible solution is to choose $\sigma$ strictly larger than $\eta$. In this particular example we can do all calculations but this is generally not the case. 
...
\section{Safe guarding (discrete time case)}
...
\begin{ex} We return to the stochastic volatility example of Section \ref{sec:stochvol}. Straightforward computation gives
\[ c = \phi(v; \mu+\omega+\psi x_0; \sigma^2+\eta^2). \]
Moreover, $I(x_0, \epsilon)$ can be  numerically approximated using Gauss-Hermite quadrature. In Figure \ref{fig:stochvol_eta1} and Figure \ref{fig:stochvol_eta3} we plot $\bar{m}$ for $\eta=1$ and $\eta=3$ respectively. In the former case we know $\bar{m}(0)<\infty$ while in the latter case $\lim_{\epsilon\downarrow 0} \bar{m}(\epsilon)=\infty$. In both cases $\bar m$ has a unique minimiser. 
...
\begin{figure}
	...
	\end{center}
	\caption{Visualisation int he stochastic volatility example of $\epsilon\mapsto \bar{m}$ when $\eta=1$.\label{fig:stochvol_eta1}}
\end{figure}
...
\begin{figure}
	...
	\end{center}
		\caption{Visualisation int he stochastic volatility example of $\epsilon\mapsto \bar{m}$ when $\eta=3$.\label{fig:stochvol_eta3}}
\end{figure}
...
\subsection{Importance sampling quality: KL and $\chi^2$ divergences}



The natural divergences for assessing importance sampling  quality of $\hat L$ are the KL divergence and the
...
\begin{proof}
\emph{Strictly decreasing:} $I'(\epsilon) = -J(\epsilon) < 0$.

\emph{Strictly convex:} $I''(\epsilon) = 2\int h^2/(g+\epsilon)^3\,\dd P > 0$.

\emph{Log-convex:} By the arithmetic-geometric inequality,
...
We return to the model from Section \ref{sec:stochvol} with parameters $\omega=0$, $\psi=0.9$, $\mu=-1.27$, $\sigma^2=\pi^2/2$.
...
%
%\noindent
To assess performance, we ran Algorithm~\ref{alg:guided_pf} independently for  $R=500$
times on the same simulated dataset with $t=0,\ldots, T$, with $T=50$. We ran the sampler with $N=2000$ particles \note{check in code, perhaps 500?} and record both the minimum
...
Figure~\ref{fig:miness_cdf} shows the empirical cumulative distribution function (cdf) of $\mathrm{ESS}_{\min}$
across $R=500$ runs for each value of $\epsilon$, separately for the two
regimes. 
\begin{itemize}
...
\begin{figure}
	\begin{center}
		\includegraphics[scale=0.5]{adaptive_miness_cdf_good.png}\\[.5in]
				\includegraphics[scale=0.6]{adaptive_miness_cdf_bad.png}\\[.5in]
			\caption{Cumulative distribution  of $\mathrm{ESS}_{\min}$ obtained from  $500$ experiments where we ran the SMC algorithm with \note{...?} particles on a fixed dataset. Top: $\eta=1$. Bottom: $\eta=3$. \label{fig:miness_cdf}}
	\end{center}
\end{figure}
...
\begin{figure}
	\begin{center}
		\includegraphics[scale=0.5]{adaptive_maxqn_cdf_good.png}\\[.5in]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Adaptive Choice of $\varepsilon_t$}
...
satisfying $g(T,x)\approx h(T,x)$. \note{explain how we  choose $g$, kolm backw eq,  $\tilde{b}(t,x)$ be the drift of a linear \emph{auxiliary process} .} Let $P^g$ denote the law of the process
...
\[
  \hat{m}(\epsilon) = (g(0,x_0)+\epsilon)
    \sum_{n=1}^{N} W^n
    \frac{\left(h(T,X_T^n\right)^2}{g(T,X_T^n)+\epsilon}
    \cdot\Psi_\epsilon(X^n),
\]
where $\Psi\epsilon(X^n)$ is evaluated by discretising~\eqref{eq:log_psi}
along the stored path:
\[
  \log\Psi_\epsilon(x^n) \approx -\sum_{k=0}^{n-1}
    \bigl(b(t_k,x_{t_k}^n)-\tilde{b}(t_k,x_{t_k}^n)\bigr)^\top
    r_\varepsilon(t_k,x_{t_k}^n)\,\Delta t.
\]
...
\section{Theoretical guarantees for the adaptive scheme (discrete-time case)}
...
\note{Nog uit te zoeken wat met onderstaande te doen.}
...
\begin{figure}
	\begin{center}
		\includegraphics[scale=0.3]{smc_run_adaptive_eta1.0.png}
		\includegraphics[scale=0.3]{smc_run_adaptive_eta3.0.png}
		\includegraphics[scale=0.3]{smc_run_fixed_eps0.0_eta1.0.png}
		\includegraphics[scale=0.3]{smc_run_fixed_eps0.0_eta3.0.png}
	\end{center}
