...
The integral appearing in $m(\eps)$ can be approximately using numerical quadrature, but in our implementation we approximate it by  drawing auxiliary proposals
...
\] Thus, this involves a Monte-Carlo estimator based on just 1 sample. 
...
Thus 
\begin{align*} m(\eps) &= \EE_{P^{g+\eps}} G_\eps(X)^2 = \int  G_\eps(x)^2 d P^{g+\eps}(x) \\ & = \int  G_\eps(x)^2 \frac{d P^{g+\eps}}{d P}(x) d P(x) \\ & = \int \left(\frac{d P^{g+\eps}}{d P}(x)\right)^{-1} h(x_T)^2 d P(x) \\ & =(g(x_0)+\eps) \int \frac{h(x_T)^2}{g(x_T)+\eps}  \Psi(x) d P(x) ,
\end{align*}
with
