...
$x_{t-1}$ and observation $v_t$, the second moment of the IS weight
$G_\varepsilon(x_{t-1}, X_t)$ when $X_t \sim P^{g_t+\varepsilon}(x_{t-1},\cdot)$
is
\[
  m(\varepsilon, x_{t-1}) =
...
\[
  m(\varepsilon) = \sum_{n=1}^N W_{t-1}^n \cdot m(\varepsilon, X_{t-1}^n)
  = \sum_{n=1}^N W_{t-1}^n\,\bigl(c(X_{t-1}^n)+\varepsilon\bigr)
    \int \frac{h_t(x_1)^2}{g_t(x_1)+\varepsilon}\,P(X_{t-1}^n,dx_1).
\]
We choose $\varepsilon_t^* = \argmin_{\varepsilon \geq 0}\, m(\varepsilon)$.


\paragraph{Empirical estimator.}
We estimate $m(\varepsilon)$ by drawing auxiliary proposals
...
\begin{itemize}
  ...
  \item In Regime A ($\eta < \sigma$), $\varepsilon_t^* \approx 0$
    typically, recovering the non-adaptive algorithm. In Regime B
    ($\eta > \sigma$), $\varepsilon_t^*$ is positive and varies over time,
    automatically responding to difficult observations.
\end{itemize}
