...
Therefore
\[ \frac{p^\star(x_{0:n})}{ p^\circ(x_{0:n})}=\frac1{h_0(x_0)} \Psi(x_{0:n}), \] where  \[ \Psi(x_{0:n})=\left[\prod_{i=1}^n k(x_i, v_i)\right]
...
\begin{lem} Define $\xi_t(x)=\log h_t(x)-\log g_t(x)$. Then 
\begin{align*}
...
\sum_{t = 0}^T\int \int  \pcirc(x_t \mid x_{t - 1}) \xi_t(x_t) \dd x_t p^\star(x_{t-1}) d x_{t-1}, \] $p^\star(x_{t-1})$ denoting the marginal density of $x_{t-1}$ under $\PP^\star_T$.
The integral in the second term an be written as
\[ -	\int \int \frac{p^\star(x_{t-1})}{p^\circ(x_{t-1})} p^\circ(x_{t-1}, x_t) \xi_t(x_t) \dd x_t \dd x_{t-1}. \]
{\it If} $\xi_t(x)\le 0$, then this can be further bounded above by 
\[ \sup_{x_{t-1}}\frac{p^\star(x_{t-1})}{p^\circ(x_{t-1})} \int p^\circ(x_t) (-\xi(x_t)) \dd x_t. \]
It is unclear whether $\xi_t(x)\le 0$ is valid,  though it  appears to be used in the bound in Equation (79) in \cite{ju2021sequential}. 
\end{rem}
